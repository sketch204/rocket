...
final class RocketParsingTests: XCTestCase {
    let sampleMarkdown = """
    ---
    tag: Swift, Programming, Blogging
    ---
    ...
    > note: If you are unfamiliar with C there is one thing that is good to know. C and Swift work a little different in terms of how they share code between source files.
    >
    ...
    Now the C library is imported and you can use all of its compatible member. This is the end of part 1. In [[From C to SwiftPM - Part 2]], we will go through what using this library is actually like, as well as some tips on how to wrap the C API behind a more Swift-friendly interface.
